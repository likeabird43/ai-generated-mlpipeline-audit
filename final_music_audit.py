...
KPOP_HITS = BASE / "data" / "kpop_hits_all_years.csv"
...
def prepare_spotify_label(spotify):
    df = spotify.copy()
...
def evaluate_label(df, label_col, label_name):
    ...
    data = df.dropna(subset=use_features + [label_col]).copy()
    ...
    wrong_model = RandomForestClassifier(n_estimators=200, random_state=42)
    ...
    metrics = {
        ...
        "wrong_cv_roc_auc": wrong_scores["test_roc_auc"].mean(),
        "wrong_cv_pr_auc": wrong_scores["test_average_precision"].mean(),
        "correct_cv_roc_auc": correct_scores["test_roc_auc"].mean(),
        "correct_cv_pr_auc": correct_scores["test_average_precision"].mean(),
...
def main():
    ...
    spotify_df, spotify_threshold = prepare_spotify_label(spotify)
    ...
    experiments = [
        ...
        (spotify_df, "hit_spotify_proxy", "spotify_popularity_proxy")
    ]
