...
KPOP_HITS_ALL_YEARS = BASE / "data" / "kpop_hits_all_years.csv"
SPOTIFY = BASE / "data" / "spotify_tracks.csv"
...
def load_data():
    ...
    hits = pd.read_csv(KPOP_HITS_ALL_YEARS)
    ...
    hit_title_col = find_column(hits, ["title", "track", "track_title", "song", "song_title"])
    hit_artist_col = find_column(hits, ["artist/s", "artist", "artists", "artist_name"])

    if hit_title_col is None:
        raise ValueError(f"Could not find title column in hit list. Columns: {hits.columns.tolist()}")

    if hit_artist_col is None:
        raise ValueError(f"Could not find artist column in hit list. Columns: {hits.columns.tolist()}")

    hits["title_norm"] = hits[hit_title_col].apply(normalize_text)
    hits["artist_list"] = hits[hit_artist_col].apply(split_artists)

    spotify_title_col = find_column(spotify, ["track_name", "title", "name", "track", "song"])
    spotify_artist_col = find_column(spotify, ["artists", "artist", "track_artist", "artist_name"])

    if spotify_title_col is not None:
        spotify["title_norm"] = spotify[spotify_title_col].apply(normalize_text)
    else:
        spotify["title_norm"] = ""

    if spotify_artist_col is not None:
        spotify["artist_norm"] = spotify[spotify_artist_col].apply(normalize_text)
    else:
        spotify["artist_norm"] = "unknown"

    return disco, hits, spotify


def create_hit_list_labels(disco, hits):
    df = disco.copy()
    ...
    df["hit_strict_all_years"] = df.apply(
        lambda r: int((r["title_norm"], r["artist_norm"]) in hit_pairs),
        axis=1,
    )

    hit_titles = set(hits["title_norm"])
    df["hit_loose_all_years"] = df["title_norm"].isin(hit_titles).astype(int)

    return df


def create_spotify_proxy_label(spotify, quantile=0.70):
    df = spotify.copy()

    if "popularity" not in df.columns:
        raise ValueError("spotify_tracks.csv must contain a popularity column")

    if "duration_ms" not in df.columns:
        raise ValueError("spotify_tracks.csv must contain duration_ms")

    threshold = df["popularity"].quantile(quantile)
    df["hit_spotify_proxy"] = (df["popularity"] >= threshold).astype(int)
...
def evaluate_label(df, label_col, label_name):
    ...
    required_cols = use_features + [label_col]

    if "artist_norm" in df.columns:
        required_cols.append("artist_norm")
    ...
    if "artist_norm" in data.columns:
        groups = data["artist_norm"]
    else:
        groups = pd.Series(["unknown"] * len(data))
...
def label_audit(kpop):
    loose_only = kpop[
        (kpop["hit_loose_all_years"] == 1) &
        (kpop["hit_strict_all_years"] == 0)
    ].copy()

    collisions = (
        kpop[kpop["hit_loose_all_years"] == 1]
        .groupby("title_norm")["Artist"]
        .nunique()
    ...
    loose_only.to_csv(OUT / "second_experiment_loose_only_matches.csv", index=False)
    collisions.to_csv(OUT / "second_experiment_title_collisions.csv", index=False)

    print("\nSaved label audit:")
    print("- second_experiment_loose_only_matches.csv")
    print("- second_experiment_title_collisions.csv")


def main():
    print("Loading data...")
    disco, hits, spotify = load_data()

    print("Creating hit-list labels...")
    kpop = create_hit_list_labels(disco, hits)

    print("Creating Spotify proxy label...")
    spotify_df, spotify_threshold = create_spotify_proxy_label(
        spotify,
        quantile=0.70,  # top 30% popularity
    )

    print("\nK-pop label counts:")
    print(kpop[["hit_strict_all_years", "hit_loose_all_years"]].sum())

    print("\nSpotify popularity threshold:", spotify_threshold)
    print("Spotify proxy positives:", spotify_df["hit_spotify_proxy"].sum())

    all_metrics = []
    all_importances = []

    experiments = [
        (kpop, "hit_strict_all_years", "strict_all_years_artist_title"),
        (kpop, "hit_loose_all_years", "loose_all_years_title_only"),
        (spotify_df, "hit_spotify_proxy", "spotify_popularity_proxy_top30"),
    ]
    ...
    for df, label_col, label_name in experiments:
        ...
        if metrics is not None:
            ...
            print("Random baseline:", round(metrics["random_pr_baseline"], 4))
    ...
    metrics_df.to_csv(OUT / "second_experiment_metrics.csv", index=False)
    importance_df.to_csv(OUT / "second_experiment_feature_importance.csv", index=False)

    label_audit(kpop)

    print("\nSaved:")
    print("- second_experiment_metrics.csv")
    print("- second_experiment_feature_importance.csv")
